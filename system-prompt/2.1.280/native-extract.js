#!/usr/bin/env node
/**
 * Extract target prompt modules from Claude Code 2.1.280 native binary.
...
// Target prompt module criteria for 2.1.280
function isTargetPromptModule(index, name, contentStr) {
  // Direct module names or indices
  if (index === 222 || name.includes("chunk-v4f98gwd") || contentStr.includes("A powerful search tool built on ripgrep")) {
    return true;
  }
  if (index === 223 || name.includes("chunk-nrvavt8a") || contentStr.includes("You are Claude Code, Anthropic")) {
    return true;
  }
  if (index === 237 || name.includes("chunk-2z7qf5nx") || contentStr.includes("The user asked for more questions")) {
    return true;
  }
  if (index === 241 || name.includes("chunk-29d79g6b") || contentStr.includes("Schedule when to resume work in /loop dynamic mode")) {
    return true;
  }
  if (index === 254 || name.includes("chunk-mqx95bs2") || contentStr.includes("Interact with Language Server Protocol")) {
    return true;
  }
  if (index === 342 || name.includes("chunk-dt8bvbsd") || (contentStr.includes("Committing changes with git") && contentStr.includes("Creating pull requests"))) {
    return true;
  }
  if (index === 456 || name.includes("chunk-wyjryvm7") || contentStr.includes("Use this tool when you are in plan mode")) {
    return true;
  }
  if (index === 472 || name.includes("chunk-7wz50e1j") || contentStr.includes("Prefer using EnterPlanMode")) {
    return true;
  }
  if (index === 531 || name.includes("chunk-9yybzjm7") || contentStr.includes("## Examples of When to Use the Todo List")) {
    return true;
  }
  return false;
}
...
// Main
const binaryPath = process.argv[2] || `${process.env.HOME}/.local/share/claude/versions/2.1.280`;
