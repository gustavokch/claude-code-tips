...
function isClaudeModule(name, content) {
  if (name.endsWith("/claude") || name === "claude" ||
      name.endsWith("/claude.exe") || name === "claude.exe" ||
      name.endsWith("/cli.js") || name.endsWith("/src/entrypoints/cli.js") ||
      name === "src/entrypoints/cli.js") {
    return true;
  }
  if (content && (content.includes("You are Claude Code, Anthropic") || content.includes("x-anthropic-billing-header"))) {
    return true;
  }
  return false;
}
...
function extract(binaryPath) {
  ...
  for (let i = 0; i < modulesCount; i++) {
    ...
    if (isClaudeModule(moduleName, moduleContent)) {
